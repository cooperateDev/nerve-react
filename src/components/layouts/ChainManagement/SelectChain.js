        ...
        <div className="">
          <div className="flex">
            ...
            {/* <div className="text-lg text-primary mb-6">
                You are currently using <span className="font-bold text-pink">Nerve</span>
                <br /> on the <span className="font-bold text-blue">{CHAIN_PARAMS[chainId].chainName}</span> network
              </div> */}
            <div
          ...
          <div className="flex flex-col space-y-3 overflow-y-auto py-2">
            {[ChainId.ETH, ChainId.BSC].map((itemChainId, i) => {
              console.log({ itemChainId, chainId });
...
function SelectSpecificNetworkButton({ chainId, isCurrentChain, onClick }) {
  const { chainLogo, chainImg, chainSymbol } = CHAIN_INFO_MAP[chainId];
  const { chainName } = CHAIN_PARAMS[chainId];
